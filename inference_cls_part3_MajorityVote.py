...
# Focal loss hyperparameters (set these to match your training config)
FOCAL_ALPHA_POS = 0.25   # alpha for positive class
FOCAL_GAMMA = 2.0        # focusing parameter gamma

# If True → save *all* columns in mv and mv_pre
# If False → save only the curated per-patient metric set
...
def run_for_checkpoint_type(
    ...
    # ------------------- Scan-level metrics per study ------------------- #
    # 1) Scan-wise correctness flag
    df_all["ScanCorrect"] = (
        df_all["Prediction"].astype(int) == df_all["GroundTruth"].astype(int)
    ).astype(int)

    # 2) Group by study_id to get scan-level summaries
    scan_group = df_all.groupby("study_id", as_index=False)

    # Number of scans per study
    scan_num = scan_group.size().rename(columns={"size": "ScanNum"})

    # Scan accuracy (fraction of scans correct) per study
    scan_acc = scan_group["ScanCorrect"].mean().rename(columns={"ScanCorrect": "ScanAccuracy"})
    # -------------------------------------------------------------------- #
    ...
    # Merge scan-level summaries into one-row-per-study DataFrame
    mv = mv.merge(scan_num, on="study_id", how="left")
    mv = mv.merge(scan_acc, on="study_id", how="left")
    ...
    # ---------------- Per-patient (study-level) scores ---------------- #
    eps = 1e-7
    probs_clipped = np.clip(probs, eps, 1.0 - eps)
    # IsCorrect: whether the MV prediction matches the ground truth
    mv["IsCorrect"] = (y_pred == y_true).astype(int)
    # Brier score: (p - y)^2
    mv["BrierScore"] = (probs - y_true) ** 2
    # Cross-entropy: -[y log p + (1-y) log(1-p)]
    mv["CrossEntropy"] = -(
        y_true * np.log(probs_clipped) + (1 - y_true) * np.log(1.0 - probs_clipped)
    )
    # Focal loss (binary), using global FOCAL_ALPHA_POS and FOCAL_GAMMA
    alpha_pos = FOCAL_ALPHA_POS
    alpha_neg = 1.0 - FOCAL_ALPHA_POS
    gamma = FOCAL_GAMMA
    # p_t: probability assigned to the true class
    p_t = np.where(y_true == 1, probs_clipped, 1.0 - probs_clipped)
    alpha_t = np.where(y_true == 1, alpha_pos, alpha_neg)
    mv["FocalLoss"] = -alpha_t * ((1.0 - p_t) ** gamma) * np.log(p_t)
    # Margin: distance from 0.5
    mv["Margin"] = probs - 0.5
    # Entropy: -[p log p + (1-p) log(1-p)]
    mv["Entropy"] = -(
        probs_clipped * np.log(probs_clipped)
        + (1.0 - probs_clipped) * np.log(1.0 - probs_clipped)
    )
    # ------------------------------------------------------------------ #
    

    # ---------------- Save per-patient metrics table ------------------- #
    if SAVE_ALL_COLUMNS:
        # Save full mv row (all columns, including metadata, probabilities, etc.)
        per_patient_df = mv.copy()
        per_patient_path = out_dir / "mv_per_patient_metrics_ALLCOLS.csv"
    else:
        # Save only the curated set of relevant columns
        per_patient_cols = [
            "study_id",
            "GroundTruth",
            "StudyLevel_Prediction",
            "StudyLevel_ProbMean",
            "StudyLevel_Error_Type",
            "ScanNum",
            "ScanAccuracy",
            "IsCorrect",
            "BrierScore",
            ...
            "Margin",
            "Entropy",]
        # Add Fold column if present
        if "Fold" in mv.columns:
            per_patient_cols.insert(5, "Fold")
        elif "fold" in mv.columns:
            per_patient_cols.insert(5, "fold")
        # keep only columns that exist
        per_patient_cols = [c for c in per_patient_cols if c in mv.columns]
        per_patient_df = mv[per_patient_cols].copy()
        per_patient_path = out_dir / "mv_per_patient_metrics.csv"
    per_patient_df.to_csv(per_patient_path, index=False)
    print(f"\n[INFO] Saved per-patient (study-level MV) metrics to: {per_patient_path}")
    # ------------------------------------------------------------------ #
    ...
    if fold_col is not None:
        ...
        if fold_cols_sorted:
            ...
            # NEW: add aggregated (pooled across all studies) as last column
            df_metrics["Aggregated"] = pd.Series({k: metrics_agg.get(k, np.nan) for k in keep}).round(4)
    ...
    # ---------------- Scan-level metrics for pre-1st subset ---------------- #
    # Per-scan correct indicator
    df_pre_slices["ScanCorrect_pre"] = (
        df_pre_slices["Prediction"].astype(int)
        == df_pre_slices["GroundTruth"].astype(int)
    ).astype(int)
    pre_scan_group = df_pre_slices.groupby("study_id", as_index=False)
    # Number of pre-1st scans for each study
    scan_num_pre = pre_scan_group.size().rename(columns={"size": "ScanNum_pre"})
    # Pre-1st scan accuracy
    scan_acc_pre = (
        pre_scan_group["ScanCorrect_pre"].mean().rename(columns={"ScanCorrect_pre": "ScanAccuracy_pre"})
    )
    # ------------------------------------------------------------------------ #
    ...
    if len(df_pre_slices) > 0:
        ...
        # Merge scan counts and scan accuracy for pre-1st subset
        per_study_pre = per_study_pre.merge(scan_num_pre, on="study_id", how="left")
        per_study_pre = per_study_pre.merge(scan_acc_pre, on="study_id", how="left")
        ...
        if fold_col_pre is not None and fold_col_pre in per_study_pre.columns:
            ...
            if fold_cols_sorted_pre:
                ...
                # NEW: add aggregated (pooled across all pre-1st studies) as last column
                df_metrics_pre["Aggregated"] = pd.Series({k: metrics_pre.get(k, np.nan) for k in keep}).round(4)
        ...
        # -------- Pre-1st per-patient scores (study-level MV from pre-1st slices) -------- #
        eps = 1e-7
        probs_p_clipped = np.clip(probs_p, eps, 1.0 - eps)
        y_true_p_arr = per_study_pre["GroundTruth"].astype(int).values
        prob_p_arr = per_study_pre["StudyLevel_ProbMean_pre"].astype(float).values
        pred_p_arr = per_study_pre["StudyLevel_Prediction_pre"].astype(int).values
        # correctness flag
        per_study_pre["IsCorrect_pre"] = (y_true_p_arr == pred_p_arr).astype(int)
        # brier
        per_study_pre["BrierScore_pre"] = (prob_p_arr - y_true_p_arr) ** 2
        # cross entropy
        per_study_pre["CrossEntropy_pre"] = -(
            y_true_p_arr * np.log(probs_p_clipped)
            + (1 - y_true_p_arr) * np.log(1.0 - probs_p_clipped))
        # focal loss
        p_t_pre = np.where(y_true_p_arr == 1, probs_p_clipped, 1.0 - probs_p_clipped)
        alpha_t_pre = np.where(y_true_p_arr == 1, FOCAL_ALPHA_POS, 1.0 - FOCAL_ALPHA_POS)
        per_study_pre["FocalLoss_pre"] = -alpha_t_pre * ((1.0 - p_t_pre) ** FOCAL_GAMMA) * np.log(p_t_pre)
        # margin
        per_study_pre["Margin_pre"] = prob_p_arr - 0.5
        # entropy
        per_study_pre["Entropy_pre"] = -(
            probs_p_clipped * np.log(probs_p_clipped)
            + (1.0 - probs_p_clipped) * np.log(1.0 - probs_p_clipped))
        ...
        # ---------------- Save pre-1st per-patient metrics ------------------- #
        if SAVE_ALL_COLUMNS:
            per_patient_pre_df = per_study_pre.copy()
            per_patient_pre_path = out_dir / "mv_pre1st_per_patient_metrics_ALLCOLS.csv"
        else:
            per_patient_pre_cols = [
                "study_id",
                "GroundTruth",
                "StudyLevel_Prediction_pre",
                "StudyLevel_ProbMean_pre",
                "ScanNum_pre",
                "ScanAccuracy_pre",
                "IsCorrect_pre",
                "BrierScore_pre",
            ...
            # Add Fold column if present
            if fold_col_pre is not None and fold_col_pre in per_study_pre.columns:
                per_patient_pre_cols.insert(5, fold_col_pre)
            per_patient_pre_cols = [
                c for c in per_patient_pre_cols if c in per_study_pre.columns]
            per_patient_pre_df = per_study_pre[per_patient_pre_cols].copy()
            per_patient_pre_path = out_dir / "mv_pre1st_per_patient_metrics.csv"
        per_patient_pre_df.to_csv(per_patient_pre_path, index=False)
        print(f"[INFO] Saved pre-1st-recurrence per-patient metrics to: {per_patient_pre_path}")

        # -------- Combined MV + pre-1st per-patient metrics -------- #
        # Select which pre1st columns we want to merge onto mv
        pre_merge_cols = [
            "study_id",
            "StudyLevel_Prediction_pre",
            "StudyLevel_ProbMean_pre",
            ...
            "Margin_pre",
            "Entropy_pre",]
        pre_merge_cols = [c for c in pre_merge_cols if c in per_study_pre.columns]
        combined_df = mv.merge(
            per_study_pre[pre_merge_cols],
            on="study_id",
            how="left",)
        if SAVE_ALL_COLUMNS:
            combined_out = combined_df
            combined_path = out_dir / "mv_and_pre1st_per_patient_metrics_ALLCOLS.csv"
        else:
            base_cols = [
                "study_id",
                "GroundTruth",
            ...
            pre_cols = [
                ...
                "Margin_pre",
                "Entropy_pre",]
            keep_cols = [c for c in (base_cols + pre_cols) if c in combined_df.columns]
            combined_out = combined_df[keep_cols]
            combined_path = out_dir / "mv_and_pre1st_per_patient_metrics.csv"
        combined_out.to_csv(combined_path, index=False)
        print(f"[INFO] Saved combined MV + pre-1st per-patient metrics to: {combined_path}")
        # ---------------------------------------------------------------- #
    else:
