...
class SimplePredictor(nnUNetPredictor):
    ...
    def initialize_from_trained_model_folder(self, model_training_output_dir: str,
        ...
        for i, f in enumerate(use_folds):
            ...
            num_classes = 1 # ckpt['classifier.3.weight'].shape[0]
    ...
    def _internal_maybe_mirror_and_predict(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if mirror_axes is not None:
            ...
            for axes in axes_combinations:
                cls_prediction_flipped = self.network(torch.flip(x, axes)) # prediction_flipped,
        ...
        return cls_prediction # prediction, 
    @torch.inference_mode()
    def _internal_predict_sliding_window_return_logits(self,
        ...
        try:
            ...
            raise e
        return class_logits # predicted_logits, 
...
if __name__ == "__main__":
    ...
    test_ids = []
    test_probs = []
    test_pred = []  # <-- collect predicted classes
    ...
    for case in tqdm(cases_dict.keys(), desc="Processing cases"):
        ...
        print(image.shape)
        cls_probs = predictor.inference(image, props, args.use_softmax) # segmentation, 
        if cls_probs.size == 1:
        # Binary head with sigmoid already applied -> cls_probs[0] = p(class 1)
            p1 = float(cls_probs[0])
            probs2 = [1.0 - p1, p1]         # [p(class 0), p(class 1)]
            pred = int(p1 >= 0.5)           # or use a tuned threshold
        else:
            # Multi-class head -> normalize and argmax
            probs2 = (cls_probs / cls_probs.sum()).tolist()
            pred = int(np.argmax(probs2))

        test_probs.append(probs2)
        ...
        # pred_cls = np.argmax(cls_probs)
    
    results = pd.DataFrame({'identifier': test_ids, 'probs': test_probs, 'pred_class': test_pred})
